                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center justify-end gap-1">
                          {club.status !== "live" && (
                            <button
                              onClick={() => handleStatusChange(club, ClubStatusEnum.LIVE)}
                              className="p-1.5 rounded-lg hover:bg-green-50 text-gray-400 hover:text-green-600 transition-colors"
                              title="Approve / Go Live"
                            >
                              <HugeiconsIcon icon={CheckmarkCircle01Icon} strokeWidth={1.5} size={16} />
                            </button>
                          )}
                          {club.status !== "hidden" && (
                            <button
                              onClick={() => handleStatusChange(club, ClubStatusEnum.HIDDEN)}
                              className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors"
                              title="Hide"
                            >
                              <HugeiconsIcon icon={ViewOffIcon} strokeWidth={1.5} size={16} />
                            </button>
                          )}
                        </div>
                      </td>
            ...
            {clubs.map((club) => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  >
                    <p className="font-semibold text-gray-900 hover:text-brand transition-colors leading-snug">
                      {club.name}
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      {formatRelativeTime(club.createdAt)}
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500 mb-3">
                  ...
                  <span className="inline-flex items-center gap-1">
                    <HugeiconsIcon icon={EyeIcon} strokeWidth={1.5} size={13} />
                    {club.clickCount ?? 0} views
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  {club.status !== "live" && (
                    <button
                      onClick={() => handleStatusChange(club, ClubStatusEnum.LIVE)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-medium bg-green-50 text-green-700 hover:bg-green-100 transition-colors"
                    >
                      <HugeiconsIcon icon={CheckmarkCircle01Icon} strokeWidth={1.5} size={14} />
                      Approve
                    </button>
                  )}
                  {club.status !== "hidden" && (
                    ...
                    >
                      <HugeiconsIcon icon={ViewOffIcon} strokeWidth={1.5} size={14} />
                      Hide
                    </button>
                  )}
                </div>
            ...
            {pagination && pagination.totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-2">
                <button
                  disabled={!pagination.hasPrev}
                  onClick={() => setPage((p) => Math.max(1, p - 1))}
                  className="px-4 py-2 text-xs font-medium rounded-xl border border-gray-200 bg-white hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  Previous
                </button>
                <span className="text-xs text-gray-500">
