  ...
  const loadUsers = useCallback(
    async (page: number, search: string, role: string) => {
      ...
      try {
        ...
        setSignupData(
          Object.entries(signupCounts).map(([name, users]) => ({ name, users }))
        );
      } catch (e) {
        toast({
          title: "Error",
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Browse, filter, and analyze user accounts.
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-sm text-muted-foreground">
              Role filter active:{" "}
              <span className="font-medium capitalize">{roleFilter}</span>
            </div>
          </CardContent>
        </Card>
      </div>
                      ...
                      <TableCell>
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full ${
                            u.auth?.emailVerified
                              ? "bg-green-100 text-green-700"
                              : "bg-gray-100 text-gray-600"
                          }`}
                        >
                          {u.auth?.emailVerified ? "Yes" : "No"}
                        </span>
                      </TableCell>
