...
export const useUrlState = (options: UseUrlStateOptions = {}) => {
  ...
  const getInitialPage = () => parseInt(searchParams.get('page') || defaultPage.toString(), 10);
  ...
  useEffect(() => {
    ...
    const urlPage = parseInt(searchParams.get('page') || defaultPage.toString(), 10);
  ...
  }, [searchParams, activeTab, currentPage, searchQuery, defaultTab, defaultPage, defaultQuery]);
